...
describe("component qualification registry", () => {
  ...
  it("registers the dormant replacement candidate with its exact frozen configuration", () => {
    const replacement = getComponentQualificationConfig(
    ...
    expect(getComponentQualificationConfigChecksum(replacement)).toBe("6356363911710657");
    expect(Object.isFrozen(replacement)).toBe(true);
    expect(() => {
      (replacement as { highImpactThreshold: number }).highImpactThreshold = 99;
    }).toThrow();
    expect(
      (
        getComponentQualificationConfig("component-impact-replacement-v1") as {
